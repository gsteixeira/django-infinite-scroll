...
function load_more() {
    ...
    if (element === null) {
        ...
        // console.log('vai chamar callback'+ window.current_pagination+' next_page '+ window.next_page+ ' has more '+ window.has_more );
        updt_from_url(window.more_url + '?'+ window.extra_args +'&page=' + window.next_page,
                        div_name,
                        load_more_callback);
    }
    return false;
